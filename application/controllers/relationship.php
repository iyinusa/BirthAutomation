...
class Relationship extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('mrelationship'); //load MODEL
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mrelationship->query_relationship_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->mrelationship->delete_relationship($del_id) > 0){
		...
		if($_POST){
			...
			if($relationship_id != ''){
				...
				if($this->mrelationship->update_relationship($relationship_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_relationship') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mrelationship->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mrelationship->query_relationship();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Relationship | NPC';
