...
class Cause extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mcause->query_cause_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->mcause->delete_cause($del_id) > 0){
		...
		if($_POST){
			...
			if($cause_id != ''){
				...
				if($this->mcause->update_cause($cause_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_cause') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mcause->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
