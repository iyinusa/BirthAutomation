...
class Lga extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mlga->query_lga_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
				$data['e_state_id'] = $item->state_id;
		...
		if($del_id != ''){
			if($this->mlga->delete_lga($del_id) > 0){
		...
		if($_POST){
			...
			if($lga_id != ''){
				...
				if($this->mlga->update_lga($lga_id, $upd_data) > 0){
				...
				if($this->user->checks('name', $name, 'state_id', $state_id, 'bz_lga') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mlga->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mlga->query_lga();
		$data['allstate'] = $this->mlga->query_state();
