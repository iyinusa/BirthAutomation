...
class Occurance extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->moccurance->query_occurance_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->moccurance->delete_occurance($del_id) > 0){
		...
		if($_POST){
			$occurance_id = $_POST['occurance_id'];
			$name = $_POST['name'];
			...
			if($occurance_id != ''){
				...
				if($this->moccurance->update_occurance($occurance_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_occurance') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->moccurance->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->moccurance->query_occurance();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Occurance | NPC';
		$data['page_act'] = 'settings';

	  	$this->load->view('designs/header', $data);
		$this->load->view('designs/leftmenu', $data);
	  	$this->load->view('occurance', $data);
