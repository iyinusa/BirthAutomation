...
class Country extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('mcountry'); //load MODEL
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mcountry->query_country_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->mcountry->delete_country($del_id) > 0){
		...
		if($_POST){
			...
			if($country_id != ''){
				...
				if($this->mcountry->update_country($country_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_country') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mcountry->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mcountry->query_country();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Country | NPC';
