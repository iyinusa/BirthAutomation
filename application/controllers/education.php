...
class Education extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('meducation'); //load MODEL
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->meducation->query_education_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->meducation->delete_education($del_id) > 0){
		...
		if($_POST){
			...
			if($education_id != ''){
				...
				if($this->meducation->update_education($education_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_education') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->meducation->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->meducation->query_education();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Education | NPC';
