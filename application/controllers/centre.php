...
class Centre extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mcentre->query_centre_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
				$data['e_lga_id'] = $item->lga_id;
		...
		if($del_id != ''){
			if($this->mcentre->delete_centre($del_id) > 0){
		...
		if($_POST){
			...
			if($centre_id != ''){
				...
				if($this->mcentre->update_centre($centre_id, $upd_data) > 0){
				...
				if($this->user->checks('name', $name, 'lga_id', $lga_id, 'bz_centre') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mcentre->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mcentre->query_centre();
		$data['alllga'] = $this->mcentre->query_lga();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Centre | NPC';
		$data['page_act'] = 'settings';

	  	$this->load->view('designs/header', $data);
		$this->load->view('designs/leftmenu', $data);
	  	$this->load->view('centre', $data);
