...
class State extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mstate->query_state_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->mstate->delete_state($del_id) > 0){
		...
		if($_POST){
			...
			if($state_id != ''){
				...
				if($this->mstate->update_state($state_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_state') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->mstate->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mstate->query_state();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'State | NPC';
