...
class Occupation extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('moccupation'); //load MODEL
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->moccupation->query_occupation_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->moccupation->delete_occupation($del_id) > 0){
		...
		if($_POST){
			...
			if($occupation_id != ''){
				...
				if($this->moccupation->update_occupation($occupation_id, $upd_data) > 0){
				...
				if($this->user->check('name', $name, 'bz_occupation') > 0) {
					$data['err_msg'] = '<div class="alert alert-info"><h5>Already exist</h5></div>';
				} else {
					if($this->moccupation->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->moccupation->query_occupation();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Occupation | NPC';
